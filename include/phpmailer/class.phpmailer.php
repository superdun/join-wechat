...
* @return boolean
*/
function sendMail($array=array(), $charset="UTF-8", $smtpSecure="ssl", $attachments=array(), $smtpAuth=true, $isHtml=true) {


    if(empty($array['host']) || empty($array['port']) || empty($array['userName']) || empty($array['password']) || empty($array['receiver']) || empty($array['sender']) || empty($array['senderName']) || empty($array['subject']) || empty($array['content'])){
        exit("缺少必要参数");
    }
    ...
    $mail->Host         = $array['host'];           // SMTP 服务器
    $mail->Port         = $array['port'];           // SMTP 端口
    $mail->SMTPSecure   = $smtpSecure;              // 加密方式
    $mail->SMTPAuth     = $smtpAuth;                // 打开SMTP认证
    $mail->Username     = $array['userName'];       // 用户名
    $mail->Password     = $array['password'];       // 密码
    $mail->From         = $array['sender'];         // 发信人
    $mail->FromName     = $array['senderName'];    // 发信人别名
    $mail->AddReplyTo($array['sender']);            // 回覆人
    $mail->AddAddress($array['receiver']);          // 收信人
    ...
    $mail->Subject  = $array['subject'];	// 邮件标题
    $mail->Body     = $array['content'];	// 邮件內容
    return $mail->Send();
}
